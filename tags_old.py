...
class Pronoun:
  ...
  def __repr__(self):
    if self.gender:
      ...
      return 'PRO.REFL' if self.reflexive else 'PRO'
class Speaker:
  def __repr__(self):
    return 'SPEAKER'
class Listener:
...
class Adj(RLam):
  def __init__(self, head):
    self.head = head
  def app(self, tok):
    return NP(get_count(tok), [self]+tok.qualifiers) \
...
class D(RLam):
  def __init__(self, head, deft, count, restrict, person):
    ...
    self.person = person
  def app(self, tok):
    return DP(self.head, self.deft, get_count(tok), tok, self.restrict, self.person) \
      if tag(tok, NP) and (get_count(tok) == self.count or self.count == UNSPEC) else None
  def __str__(self):
    return self.head
  def __repr__(self):
    return 'D.'+self.head
